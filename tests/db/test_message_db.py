...
async def test_create_message_with_user_in_private_chat(
    ...
    test_private_chat = await chat_factory(name='private_test_chat', is_group=False)
    ...
    new_message = message_factory(chat=test_private_chat, sender=test_user, content=message_text)
    ...
    query = select(Message).where(Message.message_id == new_message.message_id)
