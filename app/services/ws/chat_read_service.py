...
class ChatReadService:
    ...
    async def update_read_status(self, user_id, data_in: ChatReadStatusUpdate):
        ...
        last_read_message = await self.chat_read_status_repository.mark_as_read(
            last_read_message, data_in.message_id
        )
        await self.db.commit()

        data_out = await ChatReadStatusRead(
            chat_id=chat_id,
            user_id=user_id,
